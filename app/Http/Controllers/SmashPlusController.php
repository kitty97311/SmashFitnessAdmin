...
use DB;
use DataTables;
class SmashPlusController extends Controller
{
    public function show_smash(){
        ...
        if($session)
            {
                return view('admin.SmashPlus.default');
            }
            else
            {
    ...
    public function smash_datatable(){
        $smash = SmashPlus::where('is_deleted', '0')->get();
    ...
    public function edit_smash($id, Request $request){
        $data = SmashPlus::where('id', $id)->first();
        return $data;
    }
    ...
    public function edited_smash(request $request){
        ...
        $updated = SmashPlus::where('id', $main_id)->update($smash);
        return redirect('smashplus')->with('error', 'Success Updated');
    }

    public function delete_smash(request $request){
      
        $main_id=$request->id;
        $data = SmashPlus::where('id', $main_id)->first();
        return $data;   
    }
    ...
    public function deleted_smash(request $request){
        ...
        $data=SmashPlus::find($request->get("id"));
        
        $data->delete();
       
        return redirect('smashplus')->with('error', 'Successfull Deleted'); 
    }
}